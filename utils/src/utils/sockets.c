...
int iniciar_servidor(char *puerto, t_log *logger, char *msj_server)
{
    if (logger == NULL)
    {
        fprintf(stderr, "Error: Logger no inicializado\n");
        return -1;
    }
    ...
    if (bind(socket_servidor, servinfo->ai_addr, servinfo->ai_addrlen) < 0)
    {
        if (errno == EADDRINUSE)
        {
            log_error(logger, "%s: el puerto %s ya está en uso. "
                              "Cerrá el proceso que lo ocupa o cambiá la config.",
                      msj_server, puerto);
        }
        else
        {
...
int esperar_cliente(int socket_servidor, t_log *logger)
{
    if (logger == NULL)
    {
        fprintf(stderr, "Error: Logger no inicializado en esperar_cliente\n");
        return -1;
    }
...
void atender_cliente(void *arg)
{
    ...
    while (control_key)
    {
        ...
        case PAQUETE_OP:
            ...
            break;
        case -1:
            log_trace(data->logger, "El cliente (%s) se desconecto. Terminando servidor.", data->cliente);
            control_key = 0;
            break;
        default:
...
cliente_data_t *crear_cliente_data(int fd_cliente, t_log *logger, char *cliente)
{
    ...
    if (data == NULL)
    {
        log_error(logger, "Error al asignar memoria para cliente_data_t");
        exit(EXIT_FAILURE);
    }
    data->fd = fd_cliente;
    data->logger = logger;
    ...
    {
        log_error(logger, "Error al duplicar cadena cliente");
        // free(data);
        exit(EXIT_FAILURE);
    }
    return data;
...
char *leer_string(char *buffer, int *desplazamiento)
{
    ...
    {
        fprintf(stderr, "Error: tamaño de string inválido (%d) en leer_string\n", tamanio);
        exit(EXIT_FAILURE);
    }
...
void *recibir_buffer(int *size, int socket_cliente)
{
    ...
    {
        perror("Error al recibir tamaño del buffer");
        exit(EXIT_FAILURE);
    }
    buffer = malloc(*size);
    if (buffer == NULL)
    ...
    {
        free(buffer);
        perror("Error al recibir el buffer completo");
        exit(EXIT_FAILURE);
    }
