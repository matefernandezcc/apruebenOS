...
void execute(op_code tipo_instruccion, t_instruccion* instruccion) { //meto las syscalls tambien ??
    switch (tipo_instruccion) {
        case NOOP_OP:
            log_info(cpu_log, "INSTRUCCION :%s", tipo_instruccion); // warning: format ‘%s’ expects argument of type ‘char *’, but argument 3 has type ‘unsigned int’
            func_noop();
            break;
        case WRITE_OP:
            log_info(cpu_log, "INSTRUCCION :%s - PARAMETRO 1: %s - PARAMETRO 2: %s", tipo_instruccion, instruccion->parametros2, instruccion->parametros3); // warning: format ‘%s’ expects argument of type ‘char *’, but argument 3 has type ‘unsigned int’
        ...
        case READ_OP:
            ...
            break;
        case GOTO_OP:
            log_info(cpu_log, "INSTRUCCION :%s - PARAMETRO 1: %s", tipo_instruccion, instruccion->parametros2); // warning: format ‘%s’ expects argument of type ‘char *’, but argument 3 has type ‘unsigned int’
