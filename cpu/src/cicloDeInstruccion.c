...
void func_write(char* direccion_logica_str, char* datos) {
    ...
    if (cache_habilitada() && cache_contiene_pagina(frame)) {   // warning: implicit declaration of function ‘cache_habilitada’; did you mean ‘tlb_habilitada’? / warning: implicit declaration of function ‘cache_contiene_pagina’
        cache_escribir(frame, desplazamiento, datos);   // warning: implicit declaration of function ‘cache_escribir’
