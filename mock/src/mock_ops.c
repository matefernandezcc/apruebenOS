...
void FINALIZAR_PROC(int pid) {
    ...
    if (lista_respuesta == NULL) {
        log_error(mock_log, "Error al recibir respuesta de memoria para FINALIZAR_PROC");
        return;
    }
    ...
    if (respuesta->codigo_operacion == FINALIZAR_PROC_OP) {
        ...
        log_error(mock_log, "Error al finalizar proceso en memoria");
    }
...
void DUMP_MEMORY(int pid) {
    ...
    t_list* lista_respuesta = recibir_paquete(fd_memoria);
    if (lista_respuesta == NULL) {
        log_error(mock_log, "Error al recibir respuesta de memoria para DUMP_MEMORY");
        return;
    }
...
bool CHECK_MEMORY_SPACE(int tamanio) {
    ...
    t_list* lista_respuesta = recibir_paquete(fd_memoria);
    if (lista_respuesta == NULL) {
        log_error(mock_log, "Error al recibir respuesta de memoria para CHECK_MEMORY_SPACE");
        return false;
    }
...
bool PEDIR_INSTRUCCION_OP_mock(int cliente_socket) { 
    ...
    if (lista_respuesta == NULL) {
        log_error(mock_log, "Error al recibir instrucción de memoria");
        return false;
    }
