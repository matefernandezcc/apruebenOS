...
t_resultado_memoria crear_proceso_en_memoria(int pid, int tamanio, char* nombre_archivo) {
    ...
    log_debug(logger, "[INIT_PROC_OP] PID: %d - Páginas necesarias: %d (tamaño=%d, tam_pagina=%d)", 
    ...
    log_debug(logger, "[INIT_PROC_OP] PID: %d - Iniciando asignación de %d marcos físicos", pid, paginas_necesarias);
...
t_resultado_memoria reanudar_proceso_en_memoria(int pid) {
    ...
    if (resultado == 1) {
        ...
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
        log_debug(logger, "PID: %d - Proceso reanudado exitosamente (vía manejo_swap)", pid);
        return MEMORIA_OK;
    } else {
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
...
bool actualizar_pagina_completa(int pid, int direccion_fisica, void* contenido_pagina) {
    log_debug(logger, "PID: %d - Actualizar página completa - Dir. Física: %d", pid, direccion_fisica);
