...
t_proceso_memoria* crear_proceso_memoria(int pid, int tamanio) {
    if (pid < 0) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al crear proceso: Parámetros inválidos (tamaño=%d)", pid, tamanio);
        return NULL;
    }

    t_proceso_memoria* proceso = malloc(sizeof(t_proceso_memoria));
    if (!proceso) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al asignar memoria para estructura de proceso", pid);
        return NULL;
    }
    ...
    if (!proceso->estructura_paginas) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al crear estructura de páginas", pid);
        free(proceso);
        return NULL;
    }
    ...
    if (!proceso->metricas) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al crear métricas del proceso", pid);
    ...
    if (!proceso->instrucciones) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al crear lista de instrucciones", pid);
...
t_resultado_memoria crear_proceso_en_memoria(int pid, int tamanio, char* nombre_archivo) {
    ...
    if (!nombre_archivo || strlen(nombre_archivo) == 0) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al crear proceso: Nombre de archivo inválido", pid);
        return MEMORIA_ERROR_DIRECCION_INVALIDA;
    }

    if (!sistema_memoria) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al crear proceso: Sistema de memoria no inicializado", pid);
        return MEMORIA_ERROR_MEMORIA_INSUFICIENTE;
    }

    // Verificar si el proceso ya existe
    if (obtener_proceso(pid)) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al crear proceso: Ya existe", pid);
    ...
    log_trace(logger, "[INIT_PROC_OP] PID: %d - Páginas necesarias: %d (tamaño=%d, tam_pagina=%d)", 
              pid, paginas_necesarias, tamanio, cfg->TAM_PAGINA);

    // ========== VALIDACIÓN DE MEMORIA DISPONIBLE ==========
    log_trace(logger, "[INIT_PROC_OP] crear_proceso_en_memoria: esperando mutex_frames");
    pthread_mutex_lock(&sistema_memoria->admin_marcos->mutex_frames);
    log_trace(logger, "[INIT_PROC_OP] crear_proceso_en_memoria: mutex_frames adquirido");
    int marcos_disponibles = sistema_memoria->admin_marcos->frames_libres;
    ...
    log_trace(logger, "[INIT_PROC_OP] CREAR_PROCESO: PID %d - Estado de marcos: %d libres, %d ocupados, %d total", 
             pid, marcos_disponibles, marcos_ocupados, total_marcos);
    
    if (marcos_disponibles < paginas_necesarias) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - No hay suficiente memoria física (necesita %d páginas, disponibles %d)", 
    ...
    if (!proceso) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al crear estructura de proceso", pid);
        return MEMORIA_ERROR_MEMORIA_INSUFICIENTE;
    }
    ...
    if (nombre_archivo) {
        ...
        if (!proceso->nombre_archivo) {
            log_trace(logger, "[INIT_PROC_OP] PID: %d - Error al copiar nombre de archivo", pid);
    ...
    log_trace(logger, "[INIT_PROC_OP] crear_proceso_en_memoria: esperando mutex_procesos");
    pthread_mutex_lock(&sistema_memoria->mutex_procesos);
    log_trace(logger, "[INIT_PROC_OP] crear_proceso_en_memoria: mutex_procesos adquirido");
    ...
    log_trace(logger, "[INIT_PROC_OP] PID: %d - Iniciando asignación de %d marcos físicos", pid, paginas_necesarias);
    
    t_resultado_memoria resultado_asignacion = asignar_marcos_proceso(pid);
    if (resultado_asignacion != MEMORIA_OK) {
        log_trace(logger, "[INIT_PROC_OP] PID: %d - Error en asignación de marcos: %d", pid, resultado_asignacion);
        ...
        pthread_mutex_lock(&sistema_memoria->mutex_procesos);
        log_trace(logger, "[INIT_PROC_OP] crear_proceso_en_memoria: mutex_procesos adquirido para revertir registro");
    ...
    log_trace(logger, "PID: %d - Proceso creado exitosamente:", pid);
    log_trace(logger, "   - Páginas totales: %d", paginas_necesarias);
    log_trace(logger, "   - Páginas asignadas: %d", proceso->estructura_paginas->paginas_asignadas);
    log_trace(logger, "   - Niveles de paginación: %d", cfg->CANTIDAD_NIVELES);
    log_trace(logger, "   - Marcos físicos utilizados: %d", paginas_necesarias);
...
t_resultado_memoria finalizar_proceso_en_memoria(int pid) {
    if (!sistema_memoria) {
        log_trace(logger, "Sistema de memoria no inicializado");
        return MEMORIA_ERROR_MEMORIA_INSUFICIENTE;
    }
    ...
    if (!proceso) {
        log_trace(logger, "PID: %d - Proceso no encontrado para finalizar", pid);
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
    ...
    log_trace(logger, "FINALIZAR_PROC: Imprimiendo métricas para PID %d", pid);
    imprimir_metricas_proceso(pid);
    log_trace(logger, "FINALIZAR_PROC: Métricas impresas para PID %d", pid);
    
    // Liberar todos los marcos físicos del proceso
    log_trace(logger, "FINALIZAR_PROC: Liberando marcos para PID %d", pid);
    liberar_marcos_proceso(pid);
    log_trace(logger, "FINALIZAR_PROC: Marcos liberados para PID %d", pid);

    // Liberar espacio de SWAP asociado al proceso
    log_trace(logger, "FINALIZAR_PROC: Liberando espacio de SWAP para PID %d", pid);
    liberar_espacio_swap_proceso(pid);
    log_trace(logger, "FINALIZAR_PROC: Espacio de SWAP liberado para PID %d", pid);
    
    // Actualizar estadísticas del sistema
    log_trace(logger, "FINALIZAR_PROC: Actualizando estadísticas del sistema para PID %d", pid);
    sistema_memoria->procesos_activos--;
    ...
    log_trace(logger, "FINALIZAR_PROC: Liberando instrucciones para PID %d", pid);
    destruir_instrucciones_proceso(pid);
    log_trace(logger, "FINALIZAR_PROC: Instrucciones liberadas para PID %d", pid);

    // Remover del diccionario y liberar
    log_trace(logger, "FINALIZAR_PROC: Removiendo de diccionarios para PID %d", pid);
    dictionary_remove(sistema_memoria->procesos, pid_str);
    dictionary_remove(sistema_memoria->estructuras_paginas, pid_str);
    dictionary_remove(sistema_memoria->metricas_procesos, pid_str);
    dictionary_remove(sistema_memoria->process_instructions, pid_str);  // Ahora solo remueve entrada vacía
    
    log_trace(logger, "FINALIZAR_PROC: Liberando proceso para PID %d", pid);
    destruir_proceso(proceso);
    ...
    log_trace(logger, "FINALIZAR_PROC: Retornando MEMORIA_OK para PID %d", pid);
    return MEMORIA_OK;
}
...
t_resultado_memoria suspender_proceso_en_memoria(int pid) {
    ...
    t_proceso_memoria* proceso = dictionary_get(sistema_memoria->procesos, pid_str);
    if (!proceso) {
        log_trace(logger, "PID: %d - Proceso no encontrado para suspender", pid);
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
        return MEMORIA_ERROR_PROCESO_NO_ENCONTRADO;
    }
    
    if (proceso->suspendido) {
        log_trace(logger, "PID: %d - Proceso ya está suspendido", pid);
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
        return MEMORIA_OK;
    }
    
    if (proceso->suspending) {
        log_trace(logger, "PID: %d - Proceso ya está siendo suspendido por otro hilo", pid);
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
        return MEMORIA_OK;
    }
    ...
    if (resultado != 1) {
        log_trace(logger, "PID: %d - Error al suspender proceso a SWAP", pid);
...
t_resultado_memoria reanudar_proceso_en_memoria(int pid) {
    ...
    t_proceso_memoria* proceso = dictionary_get(sistema_memoria->procesos, pid_str);
    if (!proceso) {
        log_trace(logger, "PID: %d - Proceso no encontrado para reanudar", pid);
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
        return MEMORIA_ERROR_PROCESO_NO_ENCONTRADO;
    }
    
    if (!proceso->suspendido) {
        log_trace(logger, "PID: %d - Proceso no está suspendido", pid);
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
        return MEMORIA_OK;
    }
    ...
    if (resultado == 1) {
        ...
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
        log_trace(logger, "PID: %d - Proceso reanudado exitosamente (vía manejo_swap)", pid);
        return MEMORIA_OK;
    } else {
        pthread_mutex_unlock(&sistema_memoria->mutex_procesos);
        log_trace(logger, "PID: %d - Error al reanudar proceso (vía manejo_swap)", pid);
        return MEMORIA_ERROR_IO;
    }
}
...
t_estructura_paginas* crear_estructura_paginas(int pid, int tamanio) {
    ...
    if (!estructura) {
        log_trace(logger, "PID: %d - Error al asignar memoria para estructura de paginación", pid);
        return NULL;
    }
    ...
    if (pthread_mutex_init(&estructura->mutex_estructura, NULL) != 0) {
        log_trace(logger, "PID: %d - Error al inicializar mutex de estructura", pid);
        free(estructura);
        return NULL;
    }
    ...
    if (!estructura->tabla_raiz) {
        log_trace(logger, "PID: %d - Error al crear tabla raíz", pid);
...
t_resultado_memoria configurar_entrada_pagina(t_estructura_paginas* estructura, int numero_pagina, int numero_frame) {
    ...
    if (!entrada) {
        log_trace(logger, "PID: %d - Error al crear entrada para página %d", estructura->pid, numero_pagina);
        return MEMORIA_ERROR_DIRECCION_INVALIDA;
    }
...
int acceso_tabla_paginas(int pid, int numero_pagina) {
    ...
    for (int nivel = 0; nivel < estructura->cantidad_niveles; nivel++) {
        ...
        if (indice >= estructura->entradas_por_tabla) {
            log_trace(logger, "PID: %d - Índice fuera de rango en nivel %d", pid, nivel);
        ...
        if (nivel < estructura->cantidad_niveles - 1) {
            ...
            if (!tabla_actual) {
                log_trace(logger, "PID: %d - Tabla siguiente nula en nivel %d", pid, nivel);
                pthread_mutex_unlock(&estructura->mutex_estructura);
                return -1;
            }
        } else {
...
t_resultado_memoria leer_memoria_fisica(uint32_t direccion_fisica, int tamanio, void* buffer) {
    ...
    if (direccion_fisica + tamanio > cfg->TAM_MEMORIA) {
        log_trace(logger, "Dirección física fuera de rango: %u", direccion_fisica);
        return MEMORIA_ERROR_DIRECCION_INVALIDA;
    }
...
static t_resultado_memoria liberar_marco_interno(int numero_frame) {
    ...
    if (numero_frame < 0 || numero_frame >= admin->cantidad_total_frames) {
        log_trace(logger, "Número de frame inválido: %d", numero_frame);
        return MEMORIA_ERROR_DIRECCION_INVALIDA;
    }
    ...
    if (!frame->ocupado) {
        log_trace(logger, "Intento de liberar frame ya libre: %d", numero_frame);
        return MEMORIA_ERROR_DIRECCION_INVALIDA;
    }
...
static void liberar_marcos_proceso(int pid) {
    if (!sistema_memoria || !sistema_memoria->admin_marcos) {
        log_trace(logger, "liberar_marcos_proceso: Sistema de memoria no inicializado");
        return;
    }

    log_trace(logger, "LIBERAR_MARCOS: Iniciando liberación de marcos para PID %d", pid);
    pthread_mutex_lock(&sistema_memoria->admin_marcos->mutex_frames);
    log_trace(logger, "LIBERAR_MARCOS: Mutex adquirido para PID %d", pid);

    // Log del estado actual de marcos antes de liberar
    log_trace(logger, "LIBERAR_MARCOS: Estado actual - Total frames: %d, Libres: %d, Ocupados: %d", 
    ...
    for (int i = 0; i < sistema_memoria->admin_marcos->cantidad_total_frames; i++) {
        ...
        if (frame->ocupado && frame->pid_propietario == pid) {
            log_trace(logger, "LIBERAR_MARCOS: Liberando marco %d para PID %d", i, pid);
            liberar_marco_interno(i);
            marcos_liberados++;
            log_trace(logger, "LIBERAR_MARCOS: Marco %d liberado para PID %d", i, pid);
        }
    }

    // Log del estado final de marcos después de liberar
    log_trace(logger, "LIBERAR_MARCOS: Estado final - Total frames: %d, Libres: %d, Ocupados: %d", 
              sistema_memoria->admin_marcos->cantidad_total_frames,
              sistema_memoria->admin_marcos->frames_libres,
              sistema_memoria->admin_marcos->frames_ocupados);

    log_trace(logger, "LIBERAR_MARCOS: Total de marcos liberados para PID %d: %d", pid, marcos_liberados);
    pthread_mutex_unlock(&sistema_memoria->admin_marcos->mutex_frames);
    log_trace(logger, "LIBERAR_MARCOS: Mutex liberado para PID %d", pid);
}
...
void* acceso_espacio_usuario_lectura(int pid, int direccion_fisica, int tamanio) {
    ...
    if (direccion_fisica < 0 || tamanio <= 0) {
        log_trace(logger, "PID: %d - Parámetros inválidos para lectura", pid);
        return NULL;
    }
    
    // Validar que la dirección está dentro del espacio de memoria
    if (direccion_fisica + tamanio > cfg->TAM_MEMORIA) {
        log_trace(logger, "PID: %d - Dirección fuera de rango: %d + %d > %d", 
                 pid, direccion_fisica, tamanio, cfg->TAM_MEMORIA);
        return NULL;
    }
    
    if (!proceso_existe(pid)) {
        log_trace(logger, "PID: %d - Proceso no existe para lectura", pid);
        return NULL;
    }
    ...
    if (bloquear_marco_por_pagina(pid, direccion_fisica / cfg->TAM_PAGINA, "LECTURA_ESPACIO_USUARIO")) {
        ...
        *pag = direccion_fisica / cfg->TAM_PAGINA;
        ...
        log_trace(logger, "PID: %d - No se pudo bloquear página %d para lectura", pid, direccion_fisica / cfg->TAM_PAGINA);
        return NULL;
    }
    ...
    if (numero_marco == -1) {
        ...
        log_trace(logger, "PID: %d - No se pudo obtener marco para página %d", pid, numero_pagina);
        return NULL;
    }
    ...
    if (!dato_leido) {
        ...
        desbloquear_marco_por_pagina(pid, *pag, "LECTURA_ESPACIO_USUARIO");
        list_destroy_and_destroy_elements(paginas_bloqueadas, free);
        log_trace(logger, "Error al asignar memoria para leer marco %d", numero_marco);
        return NULL;
    }
    ...
    if (!leer_contenido_marco(numero_marco, dato_leido)) {
        ...
        desbloquear_marco_por_pagina(pid, *pag, "LECTURA_ESPACIO_USUARIO");
        list_destroy_and_destroy_elements(paginas_bloqueadas, free);
        log_trace(logger, "PID: %d - Error al leer contenido del marco %d", pid, numero_marco);
        return NULL;
    }
    ...
    if (!valor_leido) {
        ...
        desbloquear_marco_por_pagina(pid, *pag, "LECTURA_ESPACIO_USUARIO");
        list_destroy_and_destroy_elements(paginas_bloqueadas, free);
        log_trace(logger, "Error al asignar memoria para valor leído");
        return NULL;
    }
...
bool acceso_espacio_usuario_escritura(int pid, int direccion_fisica, int tamanio, void* datos) {
    ...
    if (direccion_fisica < 0 || tamanio <= 0 || datos == NULL) {
        log_trace(logger, "PID: %d - Parámetros inválidos para escritura", pid);
        return false;
    }
    ...
    if (!proceso_existe(pid)) {
        log_trace(logger, "PID: %d - Proceso no existe para escritura", pid);
        return false;
    }
    ...
    if (bloquear_marco_por_pagina(pid, direccion_fisica / cfg->TAM_PAGINA, "ESCRITURA_ESPACIO_USUARIO")) {
        ...
        *pag = direccion_fisica / cfg->TAM_PAGINA;
        ...
        log_trace(logger, "PID: %d - No se pudo bloquear página %d para escritura", pid, direccion_fisica / cfg->TAM_PAGINA);
        return false;
    }
...
void* leer_pagina_completa(int pid, int direccion_base_pagina) {
    log_trace(logger, "PID: %d - Leer página completa - Dir. Base Página: %d", pid, direccion_base_pagina);
    
    // Validar que la dirección coincide con el byte 0 de una página
    if (direccion_base_pagina % cfg->TAM_PAGINA != 0) {
        log_trace(logger, "PID: %d - Dirección base %d no coincide con byte 0 de página (TAM_PAGINA=%d)", 
    ...
    if (direccion_base_pagina + cfg->TAM_PAGINA > cfg->TAM_MEMORIA) {
        log_trace(logger, "PID: %d - Página fuera de rango: %d + %d > %d", 
    ...
    // Validar que el proceso existe
    if (!proceso_existe(pid)) {
        log_trace(logger, "PID: %d - Proceso no existe para leer página completa", pid);
        return NULL;
    }
    ...
    if (bloquear_marco_por_pagina(pid, numero_pagina, "LECTURA_PAGINA_COMPLETA")) {
        ...
        *pag = numero_pagina;
        ...
        log_trace(logger, "PID: %d - No se pudo bloquear página %d para lectura completa", pid, numero_pagina);
        return NULL;
    }
    ...
    if (pagina_completa == NULL) {
        log_trace(logger, "PID: %d - Error al reservar memoria para página completa", pid);
        int* pag = list_get(paginas_bloqueadas, 0);
...
bool actualizar_pagina_completa(int pid, int direccion_fisica, void* contenido_pagina) {
    ...
    if (contenido_pagina == NULL) {
        log_trace(logger, "PID: %d - Contenido de página es NULL", pid);
        return false;
    }
    
    // Validar que la dirección coincide con el byte 0 de una página
    if (direccion_fisica % cfg->TAM_PAGINA != 0) {
        log_trace(logger, "PID: %d - Dirección física %d no coincide con byte 0 de página (TAM_PAGINA=%d)", 
    ...
    // Validar que el proceso existe
    if (!proceso_existe(pid)) {
        log_trace(logger, "PID: %d - Proceso no existe para actualizar página completa", pid);
        return false;
    }
    ...
    if (bloquear_marco_por_pagina(pid, numero_pagina, "ESCRITURA_PAGINA_COMPLETA")) {
        ...
        *pag = numero_pagina;
        ...
        log_trace(logger, "PID: %d - No se pudo bloquear página %d para escritura completa", pid, numero_pagina);
        return false;
    }
