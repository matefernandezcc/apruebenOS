...
t_administrador_marcos* crear_administrador_marcos(int cantidad_frames, int tam_pagina) {
    ...
    if (!admin) {
        log_trace(logger, "Error al asignar memoria para administrador de marcos");
        return NULL;
    }
    ...
    if (!admin->frames) {
        log_trace(logger, "Error al asignar memoria para array de frames");
        free(admin);
        return NULL;
    }
    ...
    if (!bitmap_data) {
        log_trace(logger, "Error al crear bitmap de frames");
        free(admin->frames);
        free(admin);
        return NULL;
    }
    ...
    if (!admin->bitmap_frames) {
        log_trace(logger, "Error al inicializar bitarray");
        free(bitmap_data);
        free(admin->frames);
        free(admin);
        return NULL;
    ...
    if (pthread_mutex_init(&admin->mutex_frames, NULL) != 0) {
        log_trace(logger, "Error al inicializar mutex de frames");
...
int asignar_marco_libre(int pid, int numero_pagina) {
    ...
    if (admin->frames_libres == 0 || list_is_empty(admin->lista_frames_libres)) {
        log_trace(logger, "## No hay marcos libres disponibles - PID: %d, Página: %d", pid, numero_pagina);
...
t_resultado_memoria liberar_marco(int numero_frame) {
    ...
    if (!frame->ocupado) {
        log_trace(logger, "Intento de liberar frame ya libre: %d (FORZANDO LIBERACIÓN DE TODAS FORMAS)", numero_frame);
...
t_administrador_swap* crear_administrador_swap(void) {
    ...
    if (!admin) {
        log_trace(logger, "Error al asignar memoria para administrador de SWAP");
        return NULL;
    }
    ...
    if (admin->fd_swap == -1) {
        log_trace(logger, "Error al crear archivo de SWAP: %s", admin->path_archivo);
    ...
    if (ftruncate(admin->fd_swap, admin->tamanio_swap) == -1) {
        log_trace(logger, "Error al dimensionar archivo de SWAP");
        close(admin->fd_swap);
        free(admin->path_archivo);
        free(admin);
        return NULL;
    ...
    if (!admin->entradas) {
        log_trace(logger, "Error al crear entradas de SWAP");
        close(admin->fd_swap);
        free(admin->path_archivo);
        free(admin);
        return NULL;
    ...
    if (pthread_mutex_init(&admin->mutex_swap, NULL) != 0) {
        log_trace(logger, "Error al inicializar mutex de SWAP");
...
t_resultado_memoria inicializar_sistema_memoria(void) {
    ...
    if (!sistema_memoria) {
        log_trace(logger, "Error al asignar memoria para sistema_memoria");
        return MEMORIA_ERROR_MEMORIA_INSUFICIENTE;
    }
    ...
    if (!sistema_memoria->memoria_principal) {
        log_trace(logger, "Error al asignar memoria principal de %d bytes", cfg->TAM_MEMORIA);
    ...
    if (!sistema_memoria->admin_marcos) {
        log_trace(logger, "Error al crear administrador de marcos");
    ...
    if (!sistema_memoria->procesos || !sistema_memoria->estructuras_paginas || 
        ...
        log_trace(logger, "Error al crear diccionarios del sistema");
    ...
    if (pthread_mutex_init(&sistema_memoria->mutex_sistema, NULL) != 0 ||
        ...
        log_trace(logger, "Error al inicializar mutex del sistema");
        finalizar_sistema_memoria();
        return MEMORIA_ERROR_MEMORIA_INSUFICIENTE;
    }
    ...
    if (!sistema_memoria->admin_swap) {
        log_trace(logger, "Error al crear administrador de SWAP");
        finalizar_sistema_memoria();
        return MEMORIA_ERROR_MEMORIA_INSUFICIENTE;
    }
