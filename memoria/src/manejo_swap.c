...
int suspender_proceso_completo(int pid) {
    ...
    if (!proceso_existe(pid)) {
        log_trace(logger, "[SWAP] PID: %d - Proceso no existe, no se puede suspender", pid);
        return 0;
    }
    ...
    if (proceso == NULL) {
        log_trace(logger, "[SWAP] PID: %d - Proceso no encontrado", pid);
        return 0;
    }
    ...
    if (estructura == NULL) {
        log_trace(logger, "[SWAP] PID: %d - No se encontró estructura de páginas", pid);
    ...
    if (!asignar_espacio_swap_proceso(pid)) {
        log_trace(logger, "[SWAP] PID: %d - No hay suficiente espacio en SWAP", pid);
        pthread_mutex_unlock(&sistema_memoria->admin_swap->mutex_swap);
        return 0;
    }
    ...
    for (int i = 0; i < estructura->paginas_totales && suspension_exitosa; i++) {
        ...
        if (entrada != NULL) {
            ...
            if (contenido_pagina == NULL) {
                log_trace(logger, "[SWAP] Error al asignar memoria para página %d del proceso %d", i, pid);
            ...
            if (leer_pagina_memoria(entrada->numero_frame, contenido_pagina) != MEMORIA_OK) {
                log_trace(logger, "[SWAP] Error al leer página %d del proceso %d", i, pid);
            ...
            if (!escribir_pagina_proceso_swap(pid, i, contenido_pagina)) {
                log_trace(logger, "[SWAP] Error al escribir página %d del proceso %d a SWAP", i, pid);
                free(contenido_pagina);
                suspension_exitosa = false;
                break;
            }
    ...
    if (marcos_liberados != paginas_escritas) {
        log_trace(logger, "[SWAP][CHECK] PID: %d - Marcos liberados (%d) != páginas escritas a swap (%d)", pid, marcos_liberados, paginas_escritas);
    } else {
        log_trace(logger, "[SWAP][CHECK] PID: %d - Marcos liberados correctamente: %d", pid, marcos_liberados);
    }
    if (paginas_con_presente > 0 || paginas_con_frame > 0) {
        log_trace(logger, "[SWAP][CHECK] PID: %d - %d páginas quedaron con presente=true y %d con numero_frame!=0 tras suspensión", pid, paginas_con_presente, paginas_con_frame);
    } else {
        log_trace(logger, "[SWAP][CHECK] PID: %d - Todas las entradas de página correctamente con presente=false y numero_frame=0", pid);
    }
    if (suspension_exitosa) {
        proceso->suspendido = true;
        incrementar_bajadas_swap(pid);
        log_trace(logger, "[SWAP] PID: %d - Proceso suspendido completamente. Páginas escritas a SWAP: %d, omitidas: %d", pid, paginas_escritas, paginas_omitidas);
        
    } else {
        log_trace(logger, "[SWAP] PID: %d - Falló la suspensión, liberando páginas de SWAP", pid);
...
int reanudar_proceso_suspendido(int pid) {
    if (!proceso_existe(pid)) {
        log_trace(logger, "PID: %d - Proceso no existe, no se puede reanudar", pid);
        return 0;
    }
    ...
    t_estructura_paginas* estructura = proceso->estructura_paginas;
    if (estructura == NULL) {
        log_trace(logger, "PID: %d - No se encontró estructura de páginas", pid);
        pthread_mutex_unlock(&sistema_memoria->admin_swap->mutex_swap);
        return 0;
    }
    ...
    if (sistema_memoria->admin_marcos->frames_libres < estructura->paginas_totales) {
        log_trace(logger, "PID: %d - No hay suficiente espacio en memoria para reanudar proceso", pid);
        pthread_mutex_unlock(&sistema_memoria->admin_swap->mutex_swap);
        return 0;
    }
    ...
    for (pagina_actual = 0; pagina_actual < estructura->paginas_totales && reanudacion_exitosa; pagina_actual++) {
        log_trace(logger, "PID: %d - [REANUDAR] INICIO iteración página %d de %d", pid, pagina_actual, estructura->paginas_totales);
        t_entrada_tabla* entrada = buscar_entrada_tabla(estructura, pagina_actual);
        
        if (entrada != NULL && !entrada->presente) {
            log_trace(logger, "PID: %d - [REANUDAR] Antes de asignar_marco_libre para página %d", pid, pagina_actual);
            // Asignar nuevo frame en memoria principal
            int nuevo_frame = asignar_marco_libre(pid, pagina_actual);
            log_trace(logger, "PID: %d - [REANUDAR] Después de asignar_marco_libre para página %d (frame=%d)", pid, pagina_actual, nuevo_frame);
            if (nuevo_frame == -1) {
                log_trace(logger, "PID: %d - No se pudo asignar frame para página %d", pid, pagina_actual);
            ...
            log_trace(logger, "PID: %d - [REANUDAR] Antes de leer_pagina_proceso_swap para página %d", pid, pagina_actual);
            void* contenido = leer_pagina_proceso_swap(pid, pagina_actual);
            log_trace(logger, "PID: %d - [REANUDAR] Después de leer_pagina_proceso_swap para página %d (contenido=%p)", pid, pagina_actual, contenido);
            if (contenido == NULL) {
                log_trace(logger, "PID: %d - Error al leer página %d desde SWAP", pid, pagina_actual);
            ...
            log_trace(logger, "PID: %d - [REANUDAR] Antes de escribir_pagina_memoria para página %d", pid, pagina_actual);
            if (escribir_pagina_memoria(nuevo_frame, contenido) != MEMORIA_OK) {
                log_trace(logger, "PID: %d - Error al escribir página %d en memoria", pid, pagina_actual);
                free(contenido);
                entrada->presente = false;
                entrada->numero_frame = 0;
                liberar_marco(nuevo_frame);
            ...
            log_trace(logger, "PID: %d - [REANUDAR] Después de escribir_pagina_memoria para página %d", pid, pagina_actual);
            paginas_cargadas++;
            free(contenido);
            log_trace(logger, "PID: %d - Página %d reanudada exitosamente en marco %d", pid, pagina_actual, nuevo_frame);
        } else if (entrada == NULL) {
            log_trace(logger, "PID: %d - Entrada de tabla nula para página %d al reanudar", pid, pagina_actual);
            paginas_fallidas++;
        } else if (entrada->presente) {
            log_trace(logger, "PID: %d - Página %d ya presente en memoria al reanudar", pid, pagina_actual);
        }
        log_trace(logger, "PID: %d - [REANUDAR] FIN iteración página %d", pid, pagina_actual);
    }
    ...
    if (reanudacion_exitosa) {
        ...
        pthread_mutex_unlock(&sistema_memoria->admin_swap->mutex_swap);
        liberar_espacio_swap_proceso(pid);
        // No es necesario volver a tomar el mutex aca
        ...
        log_trace(logger, "PID: %d - Proceso reanudado exitosamente. Páginas cargadas desde SWAP: %d, fallidas: %d", 
                 pid, paginas_cargadas, paginas_fallidas);
    } else {
        // Si falló la reanudación, liberar los marcos ya asignados en esta operación
        log_trace(logger, "PID: %d - Falló la reanudación, liberando marcos asignados", pid);
...
int escribir_pagina_proceso_swap(int pid, int numero_pagina, void* contenido) {
    ...
    if (!proceso || !proceso->estructura_paginas) {
        log_trace(logger, "PID: %d - Proceso no encontrado para escribir página %d", pid, numero_pagina);
    ...
    if (cfg->TAM_PAGINA > sizeof(buffer_pagina)) {
        log_trace(logger, "Tamaño de página (%d) excede el buffer (%d)", cfg->TAM_PAGINA, (int)sizeof(buffer_pagina));
        // desbloquear_marco_por_pagina(pid, numero_pagina, "ESCRITURA_SWAP");
        return 0;
    }
    ...
    if (entrada_swap == -1) {
        log_trace(logger, "PID: %d - No hay entradas libres en SWAP para registrar página %d", pid, numero_pagina);
        return 0;
    }
    ...
    if (lseek(sistema_memoria->admin_swap->fd_swap, offset, SEEK_SET) == -1) {
        log_trace(logger, "Error al posicionarse en SWAP para escribir página %d del proceso %d: %s", numero_pagina, pid, strerror(errno));
        return 0;
    }
    ...
    if (bytes_escritos != cfg->TAM_PAGINA) {
        log_trace(logger, "Error al escribir página a SWAP: solo se escribieron %zd de %d bytes. errno=%d (%s)", 
    ...
    log_trace(logger, "PID: %d - Página %d escrita en SWAP en entrada %d (offset %ld)", pid, numero_pagina, entrada_swap, offset);
    
    // Actualizar información de SWAP
    entrada->presente = false;  // Ya no está en memoria principal
    entrada->timestamp_acceso = time(NULL);
    
    // NO incrementar métrica aca - se hace una sola vez por proceso en suspender_proceso_completo
    // incrementar_bajadas_swap(pid);
...
void* leer_pagina_proceso_swap(int pid, int numero_pagina) {
    ...
    if (posicion_swap == -1) {
        log_trace(logger, "PID: %d - Página %d no encontrada en SWAP", pid, numero_pagina);
        // pthread_mutex_unlock(&sistema_memoria->admin_swap->mutex_swap);
        return NULL;
    }
    ...
    if (contenido == NULL) {
        log_trace(logger, "PID: %d - Error al asignar memoria para leer página %d", pid, numero_pagina);
        // pthread_mutex_unlock(&sistema_memoria->admin_swap->mutex_swap);
        return NULL;
    }
    ...
    if (lseek(sistema_memoria->admin_swap->fd_swap, offset, SEEK_SET) == -1) {
        log_trace(logger, "PID: %d - Error al posicionarse en SWAP para leer página %d", pid, numero_pagina);
        free(contenido);
        // pthread_mutex_unlock(&sistema_memoria->admin_swap->mutex_swap);
        return NULL;
    }
    if (read(sistema_memoria->admin_swap->fd_swap, contenido, cfg->TAM_PAGINA) != cfg->TAM_PAGINA) {
        log_trace(logger, "PID: %d - Error al leer página %d desde SWAP", pid, numero_pagina);
        free(contenido);
        // pthread_mutex_unlock(&sistema_memoria->admin_swap->mutex_swap);
        return NULL;
    }
...
int asignar_espacio_swap_proceso(int pid) {
    ...
    if (proceso == NULL) {
        log_trace(logger, "PID: %d - Proceso no encontrado para asignar SWAP", pid);
        return 0;
    }
    ...
    if (sistema_memoria->admin_swap->paginas_libres_swap < paginas_necesarias) {
        log_trace(logger, "PID: %d - No hay suficiente espacio en SWAP (%d páginas necesarias, %d disponibles)", 
