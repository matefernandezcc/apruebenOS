...
############################
# INICIAR MEMORIA
valgrind --leak-check=full --log-file=memoria/memoria.valgrind ./memoria/bin/memoria &
PID_MEMORIA=$!
sleep 10

# INICIAR KERNEL
valgrind --leak-check=full --log-file=kernel/kernel.valgrind ./kernel/bin/kernel proceso_inicial 128 kernel.config --action &
PID_KERNEL=$!
sleep 10

# INICIAR CPU
valgrind --leak-check=full --log-file=cpu/cpu.valgrind ./cpu/bin/cpu 1 &
PID_CPU1=$!
sleep 10

# INICIAR IO
valgrind --leak-check=full --log-file=io/io.valgrind ./io/bin/io teclado &
PID_IO1=$!
sleep 10
